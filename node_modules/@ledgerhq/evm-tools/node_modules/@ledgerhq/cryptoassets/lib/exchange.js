"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.findExchangeCurrencyConfig = void 0;
const erc20_1 = __importDefault(require("./data/exchange/erc20"));
const coins_1 = __importDefault(require("./data/exchange/coins"));
const bep20_1 = __importDefault(require("./data/exchange/bep20"));
const trc20_1 = __importDefault(require("./data/exchange/trc20"));
const polygon_erc20_1 = __importDefault(require("./data/exchange/polygon-erc20"));
const all = [...coins_1.default, ...erc20_1.default, ...bep20_1.default, ...trc20_1.default, ...polygon_erc20_1.default];
const configs = {};
for (const [id, config, signature] of all) {
    configs[id] = {
        config,
        signature,
    };
}
/**
 *
 */
const findExchangeCurrencyConfig = (id) => configs[id];
exports.findExchangeCurrencyConfig = findExchangeCurrencyConfig;
//# sourceMappingURL=exchange.js.map